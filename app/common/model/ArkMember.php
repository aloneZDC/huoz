...
class ArkMember extends Model
{
    ...
    static function addItem($member_id, $num) {
        ...
        try{
            ...
            $type = 1;
            $valid = ArkConfig::getValue('valid_username');//获取成为有效账户金额
            if($info) {
                if ($num > 0) {
    ...
    static function updateData($member_id, $num, $type) {
        ...
        // 更新团队业绩
        $flag = self::execute('update ' . config("database.prefix") . 'ark_member a,' . config("database.prefix") . 'member_bind b
    ...
    static function updateLevel($member_id) {
        ...
        $membersLevel = ArkLevel::where(['valid_num' => ['elt', $member_info['valid_num']], 'active_min' => ['elt', $active_min]])->order(['id' => 'DESC'])->find();
        $flag = true;
        if ($membersLevel) {
    ...
    static function getActiveMaxCount($member_id) {
        $result = MemberBind::alias('a')
            ->join(config("database.prefix") . "ark_member b", 'a.child_id = b.member_id')
    ...
    static function get_pay_info($member_id, $product_id, $money) {
        ...
        $goods_info = ArkGoodsList::where(['id' => $product_id])->find();
        ...
        $num = sprintf('%.4f', $money * ($goods_info['kmt_rate'] / 100));
...
//        }
    ...
    static function get_community_info($member_id) {
        ...
        $result = self::where(['member_id' => $member_id])->field('level,total_num,team_total,valid_num,share_reward,team_reward')->find();
        if (!$result) return $r;
        ...
        $result['level'] = 'Y' . $result['level'];
        $one_num = Db::name('member')->where(['pid' => $member_id])->count('member_id');
        $result['one_num'] = $one_num ?: 0;
//        $valid_num = ArkRewardLog::getValidNum($member_id);
//        $result['valid_num'] = $valid_num ?: 0;
        $today_start = strtotime(date("Y-m-d"));
        $today_end = $today_start + 86399;
        $today_share_reward = ArkRewardLog::where(['member_id' => $member_id, 'type' => 2, 'add_time' => ['between', [$today_start, $today_end]]])->sum('reward');
        $result['today_share_reward'] = $today_share_reward ? sprintf('%.2f', $today_share_reward): 0;
        $today_team_reward = ArkRewardLog::where(['member_id' => $member_id, 'type' => 3, 'add_time' => ['between', [$today_start, $today_end]]])->sum('reward');
...
//        $yestday_end = $yestday_start + 86399;
        $is_seniority = Db::name('ark_summary')->where(['member_id' => $member_id])->order('id desc')->value('is_reward');
        $result['is_seniority'] = !empty($is_seniority) ? 1 : 0;//昨日本人是否合格 1是 0否
        $first_reward = ArkRewardLog::alias('a')
    ...
    static function getMaxSubscribe($member_id) {
        ...
        if ($res) {
            ...
            foreach ($res as $key => $value) {
                $team_num = ArkSubscribeTransfer::alias('a')->join('member_bind b', 'a.member_id=b.child_id')->where(['b.member_id' => $value])->sum('a.num');
                $num = ArkSubscribeTransfer::where(['member_id' => $value])->sum('num');
...
//            $valid_num = ArkRewardLog::getValidNum($value['member_id']);
//            $value['valid_num'] = $valid_num;
            $member_info = Db::name('member')->where(['member_id' => $value['member_id']])->find();
            $value['level'] = 'Y' . $value['level'];
            $value['count_num'] = sprintf('%.2f', $value['total_num'] + $value['team_total']);
            $value['ename'] = $member_info['ename'];
            $value['add_time'] = date('Y-m-d H:i:s', $value['add_time']);
            $is_seniority = Db::name('ark_summary')->where(['member_id' => $value['member_id']])->order('id desc')->value('is_reward');
            $value['is_seniority'] = !empty($is_seniority) ? 1 : 0;//昨日本人是否合格 1是 0否

            $yestday_reward = ArkRewardLog::where(['member_id' => $value['member_id'], 'type' => ['in', [2,3,5,6,7,8]], 'add_time' => ['between', [$today_start, $today_end]]])->sum('reward') ?: 0.00;
            $total_reward = ArkRewardLog::where(['member_id' => $value['member_id'], 'type' => ['in', [2,3,5,6,7,8]]])->sum('reward') ?: 0.00;
            $value['reward'] = sprintf('%.4f', $yestday_reward) .' / '. sprintf('%.4f', $total_reward);//昨日本人收益/累计收益

            $yestday_team = ArkRewardLog::alias('a')
                ->join('member_bind b', 'a.member_id=b.child_id')
                ->where(['b.member_id' => $value['member_id'], 'a.type' => 3, 'a.add_time' => ['between', [$today_start, $today_end]]])
                ->sum('a.third_money') ?: 0;
            $total_team = ArkRewardLog::alias('a')
                ->join('member_bind b', 'a.member_id=b.child_id')
                ->where(['b.member_id' => $value['member_id'], 'a.type' => 3])
                ->sum('a.third_money') ?: 0;
            $value['team_reward'] = sprintf('%.2f', $yestday_team) .' / '. sprintf('%.2f', $total_team);//昨日团队流水/累计团队流水

            $yestday_num = ArkRewardLog::where(['member_id' => $value['member_id'], 'type' => 3, 'add_time' => ['between', [$today_start, $today_end]]])->sum('third_money') ?:0;
            $total_num = ArkRewardLog::where(['member_id' => $value['member_id'], 'type' => 3])->sum('third_money') ?: 0;
            $value['personal_num'] = sprintf('%.2f', $yestday_num) .' / '. sprintf('%.2f', $total_num);//昨日个人流水/累计团队流水

            $yestday_personal_subscribe = ArkSubscribeTransfer::where(['member_id' => $value['member_id'], 'add_time' => ['between', [$yestday_start, $yestday_end]]])->sum('num') ?: 0;
            $personal_subscribe = ArkSubscribeTransfer::where(['member_id' => $value['member_id']])->sum('num') ?: 0;
            $value['personal_subscribe'] = sprintf('%.2f', $yestday_personal_subscribe) .' / '. sprintf('%.2f', $personal_subscribe);//昨日个人预充/累计预充

            $yestday_team_subscribe = ArkSubscribeTransfer::alias('a')->join('member_bind b', 'a.member_id=b.child_id')->where(['b.member_id' => $value['member_id'], 'add_time' => ['between', [$yestday_start, $yestday_end]]])->sum('a.num');
            $team_subscribe = ArkSubscribeTransfer::alias('a')->join('member_bind b', 'a.member_id=b.child_id')->where(['b.member_id' => $value['member_id']])->sum('a.num');
            $value['team_subscribe'] = sprintf('%.2f', $yestday_team_subscribe) .' / '. sprintf('%.2f', $team_subscribe);//昨日团队预充/累计预充

            $total_personal_reward = ArkRewardLog::where(['member_id' => $value['member_id'], 'type' => 1])->sum('third_money');
            $value['total_personal_reward'] = sprintf('%.2f', $total_personal_reward);//累计本人流水
            $total_team_reward = ArkRewardLog::alias('a')
                ->join('member_bind b', 'a.member_id=b.child_id')
    ...
    static function set_fire($member_id, $is_fire, $quota_type, $quota_price = 0) {
        ...
        $flag = ArkMember::where(['member_id' => $member_id])->update($data);
    ...
    static function auto_fire($data) {
        ...
        foreach ($res as $key => $value) {
            ...
            //添加订单
            $flag = ArkOrder::add_order($value['member_id'], $data['id'], $num, $kmt_num);
    ...
    public function rocketLevel() {
        return $this->belongsTo('app\\common\\model\\ArkLevel', 'level', 'level')->field('level');
    }
    ...
    static function get_subscribe_num($member_id) {
        ...
        $self_num = Db::name('ark_subscribe_transfer')->where(['member_id' => $member_id, 'add_time' => ['between', [$today_start, $today_end]], 'type' => 1])->sum('num');
        $result['self_num'] = $self_num ? sprintf('%.2f', $self_num) : 0;
        $total_self_num = Db::name('ark_subscribe_transfer')->where(['member_id' => $member_id, 'type' => 1])->sum('num');
        ...
        if ($small_ids) {
            foreach ($small_ids as $item) {
                $frist = $second  = 0;
                $one = Db::name('ark_subscribe_transfer')->where(['member_id' => $item, 'add_time' => ['between', [$today_start, $today_end]], 'type' => 1])->sum('num');
                $frist = Db::name('ark_subscribe_transfer')->alias('a')
                    ->join('member_bind b', 'a.member_id = b.child_id')
                    ->where(['b.member_id' => $item, 'a.add_time' => ['between', [$today_start, $today_end]], 'a.type' => 1])
                    ->sum('num');
                $arr[] = sprintf('%.2f', $frist + $one);
                $total_one = Db::name('ark_subscribe_transfer')->where(['member_id' => $item, 'type' => 1])->sum('num');
                $second = Db::name('ark_subscribe_transfer')->alias('a')
                    ->join('member_bind b', 'a.member_id = b.child_id')
