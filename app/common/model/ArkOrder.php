...
class ArkOrder extends Model
{
    ...
    static function add_order($member_id, $product_id, $num, $kmt_num) {
        ...
        $goods_info = ArkGoodsList::where(['id' => $product_id, 'status' => 1])->find();
        ...
        try {
            ...
            if ($kmt_num > 0) {
                ...
                $flag = ArkKmtLog::addItem($platform_currency_user['member_id'], $product_id, $kmt_num);
                if (!$flag) throw new Exception('生成燃料记录失败');
            }
            //更新闯关完成金额
            //$flag = Db::name('rocket_goods_list')->where(['id' => $product_id])->lock(true)->find();

            $flag = ArkGoodsList::where(['id' => $product_id])->setInc('finish_money', $num);
            if ($flag === false) throw new Exception('参与金额不能大于闯关金额');
            //更新用户汇总
            $flag = ArkMember::addItem($member_id, $num);
    ...
    static function settlement_order($data, $today_config, $config) {
        ...
        if (time() >= $data['start_time'] && $finsh_date >= $today_config['today_start']) {
            //更新闯关时间
            $flag = ArkGoodsList::update_goods($data['id'], $today_config['today_start']);
        ...
        try {
            ...
            if (sprintf('%.2f', $finsh_money) >= sprintf('%.2f', $data['price'])) {//成功
                ...
                //结算关id
                $first_id = ArkGoodsList::where(['goods_id' => $data['goods_id'], 'level' => $level, 'status' => 2, 'rocket_status' => 2])->value('id');
                if ($level > 0 && $first_id) {
                    ...
                    $goods_info = ArkGoodsList::where(['id' => $first_id])->find();
                    if (!$goods_info) {
                        Db::commit();
                        return true;
                    }
...
//                            'share_reward' => $share_reward,
                        ...
                        //更新用户汇总
                        $flag = ArkMember::where(['member_id' => $value['member_id']])->update(['statics_reward' => ['inc', $reward]]);
                        if ($flag === false) throw new Exception("更新用户汇总失败");
                    }

                    //分享收益
//                    $member_info = ArkMember::select();
//                    $qualified = ArkConfig::getValue('check_reward_qualified');//获取奖励资格
//                    $subscribe_id = ArkConfig::getValue('subscribe_currency_id');//预约池账户
//                    $unsettled_price = ArkConfig::getValue('unsettled_price');//未结算数量
//                    foreach ($member_info as $k) {
//                        //判断用户存在参与未结算的闯关且金额小于100或者预约池账户余额小于100
...
//                        if ($is_check < $unsettled_price && $subscribe['num'] < $qualified) {
//                            continue;
//                        }
...
//                    }
                    ...
                    if ($goods_info['lucky_Jackpot'] > 0) {
                        ...
                        $lucky_reward = sprintf('%.2f', ($finsh_money * ($goods_info['lucky_Jackpot'] / 100)) * $initial_price);
                    }
                    //基金
                    $fund_reward = 0;
                    ...
                    //开发结算
                    if ($finsh_money > 0) {
                        $flag = self::handle_development($finsh_money);
                        if ($flag === false) throw new Exception("开发结算失败");
                    }
                    //更新主商品数据
                    $goods_id = ArkGoodsList::where(['id' => $first_id])->value('goods_id');
                    $gdata = [
                        'warehouse1' => ['inc', $lucky_reward],
                        'warehouse2' => ['inc', $fund_reward],
                        'warehouse3' => ['inc', $platform_reward],
                    ];
                    $flag = ArkGoods::where(['id' => $goods_id])->update($gdata);
                    ...
                    $flag = ArkGoodsList::where(['id' => $first_id])->update($gldata);
                    if ($flag === false) throw new Exception("更新用户汇总失败");

                    //更新闯关数据(已结算)
                    $flag = ArkGoodsList::update_goods($data['id'], $today_config['today_start'], 2);
                ...
                //更新状态
                $flag = ArkGoodsList::where(['id' => $data['id']])->update(['status' => 2]);
                if ($flag === false) throw new Exception("更新状态失败");
            } else {//失败
                $goods_ids = ArkGoodsList::where(['goods_id' => $data['goods_id'], 'rocket_status' => ['in', [0,1,2]], 'end_time' => ['elt', $today_config['today_start']], 'is_show' => 1])->column('id');
                ...
                //判断是否大于第三关（初始结算关）
                if ($data['level'] > $settlement_level) {
                    $warehouse = ArkGoods::where(['id' => $data['goods_id']])->value('warehouse1');
                    if ($res) {
                        foreach ($res as $key => $value) {
                            $integral_num = 0;
                            $integral = 0;
...
//                                $price = YefiCurrencyPrice::order('id desc')->value('price');
//                                if (!$price) {
                                    $price = ArkConfig::getValue('initial_price');
                                //}
                                //积分 * Y令牌单价 / L令牌单价
                                $integral_price = ArkConfig::getValue('initial_integral_price');//L令牌固定价格
                                $integral_num = sprintf('%.6f', ($integral * $price) / $integral_price);
                            }
                ...
                foreach ($goods_ids as $gv) {
                    //更新闯关时间
                    $flag = ArkGoodsList::update_goods($gv, $today_config['today_start'], 3);
                    if ($flag === false) throw new Exception("更新闯关时间失败");
                    //更新状态
                    $flag = ArkGoodsList::where(['id' => $gv])->update(['status' => 3]);
                    if ($flag === false) throw new Exception("更新状态失败");
                }
                //清除幸运舱
                $flag = ArkGoods::where(['id' => $data['goods_id']])->update(['warehouse1' => 0]);
    ...
    /**
     * 处理开发结算
     * @param number $num   结算金额
     */
    static function handle_development($num) {
        //获取hxy指定账户（开发方指定账户）
        $hxy_user_id = Config::get_value('hxy_user_id', '');//账户
        $hxy_rate = Config::get_value('hxy_rate', 0);//抽佣比例
        $hxy_num = 0;
        if ($hxy_user_id && $hxy_rate) {
            $hxy_rate = sprintf('%.6f', $hxy_rate / 100);
            $hxy_num = sprintf('%.6f', $num * $hxy_rate);
        }
        $reward_currency_id = ArkConfig::getValue('reward_currency_id');
        //账本类型
        $account_book_id = 7115;
        $account_book_content = 'rocket_commission';

        if ($hxy_num > 0) {//开发方抽佣每关的0.5%
            $hxy_currency_user = CurrencyUser::getCurrencyUser($hxy_user_id, $reward_currency_id);
            if (empty($hxy_currency_user)) return false;

            //增加账本 增加资产
            $flag = AccountBook::add_accountbook($hxy_currency_user['member_id'], $hxy_currency_user['currency_id'], $account_book_id, $account_book_content, 'in', $hxy_num, 1);
            if ($flag === false) throw new Exception("添加账本失败");

            $flag = CurrencyUser::where(['cu_id' => $hxy_currency_user['cu_id'], 'num' => $hxy_currency_user['num']])->setInc('num', $hxy_num);
...
//        }
    ...
    static function handle_warehouse($num, $type) {
        ...
        //获得市值舱、工具舱的用户
        $show_user_id = ArkConfig::getValue('show_user_id');
        if ($show_user_id) {
            $reward_currency_id = ArkConfig::getValue('reward_currency_id');
    ...
    static function handle_principal($member_id, $goods_list_id, $level, $id, $type, $warehouse = 0) {
        ...
        $goods_info = ArkGoodsList::where(['id' => $goods_list_id])->find();
        ...
        if ($type == 2) {
            $fail_lucky_rate = ArkConfig::getValue('fail_lucky_rate');
            $reward = sprintf('%.6f', ($money / $goods_info['price']) * ($warehouse * ($fail_lucky_rate / 100)));
        }
// elseif ($type == 3) {
//            $fail_principal_rate = ArkConfig::getValue('fail_principal_rate');
...
//        }
        ...
        try {
            if ($money > 0) {//本金
                ...
                $special_user_id = ArkConfig::getValue('special_user_id');
...
//                            $subscribe_currency_user = CurrencyUser::getCurrencyUser($member_id, $subscribe_currency_id);
//                            if ($type == 3) {
//                                $flag = ArkSubscribeTransfer::addItem($member_id, 2, $money);
...
//
//                    //增加账本 增加资产
//                    $flag = AccountBook::add_accountbook($integral_currency_user['member_id'], $integral_currency_user['currency_id'], $account_book_id, $account_book_content, 'in', $integral_num, $id);
//                    if ($flag === false) throw new Exception("添加账本失败");
//
//                    $flag = CurrencyUser::where(['cu_id' => $integral_currency_user['cu_id'], 'num' => $integral_currency_user['num']])->setInc('num', $integral_num);
//                    if ($flag === false) throw new Exception("添加资产失败");
//
//                    //质押奖励
//                    $pledge_reward_rate = ArkConfig::getValue('pledge_reward_rate');
//                    $pledge_reward = 0;
//                    if ($pledge_reward_rate) {
//                        $pledge_reward = sprintf('%.6f', $integral * ($pledge_reward_rate / 100));
//                    }
//                    if ($pledge_reward > 0) {
//                        $pledge_currency_id = ArkConfig::getValue('reward_currency_id');
//                        $pledge_currency_user = CurrencyUser::getCurrencyUser($member_id, $pledge_currency_id);
//                        $flag = AccountBook::add_accountbook($pledge_currency_user['member_id'], $pledge_currency_user['currency_id'], 7124, 'pledge_reward', 'in', $pledge_reward, $id);
//                        if ($flag === false) throw new Exception("添加账本失败");
//
//                        $flag = CurrencyUser::where(['cu_id' => $pledge_currency_user['cu_id'], 'num' => $pledge_currency_user['num']])->setInc('num', $pledge_reward);
//                        if ($flag === false) throw new Exception("添加资产失败");
//                    }
//                }
            }
            ...
            if ($reward > 0) {//幸运池加权分红
                ...
                // 收益记录
                $member_level = ArkMember::where(['member_id' => $member_id])->value('level');
                $item_id = ArkRewardLog::addItem($currency_user['member_id'], $member_level, $reward, $profit, $money, $id, 4);
                ...
                $flag = ArkGoodsList::where(['id' => $goods_list_id])->setInc('warehouse1', $reward);
                if ($flag === false) throw new Exception("添加资产失败");
            }
    ...
    static function get_statics_reward($member_id, $goods_list_id, $level, $profit, $id) {
        ...
        $money = $res['money'];
        $initial_price = ArkConfig::getValue('initial_price');
        $result = sprintf('%.6f', ($res['money'] * ($profit / 100)) * $initial_price);
        ...
        try {
            // 收益记录
            $member_level = ArkMember::where(['member_id' => $member_id])->value('level');
            $item_id = ArkRewardLog::addItem($currency_user['member_id'], $member_level, $result, $profit, $money, $id, 1);
            ...
            if ($result > 0) {
                ...
                $flag = ArkMember::where(['member_id' => $member_id])->setInc('statics_reward', $result);
    ...
    static function get_share_reward($member_id, $goods_list_id, $level) {
        $result = 0;
        $valid_num = ArkMember::where(['member_id' => $member_id])->value('valid_num');
        ...
        if ($share_config) {
            ...
            if ($share_config['first'] > 0) {//第一代投入金额的收益
                ...
                $first_reward = sprintf('%.6f', $first_total_num * ($share_config['first'] / 100));
                // 收益记录
                if ($first_reward > 0) {
                    ArkRewardLog::addItem($member_id, $member_level, $first_reward, $share_config['first'], $first_total_num, 0, 2, 1);
                }
            }
            if ($share_config['second'] > 0) {//第二代投入金额的收益
                ...
                $second_reward = sprintf('%.6f', $second_total_num * ($share_config['second'] / 100));
                // 收益记录
                if ($second_reward > 0) {
                    ArkRewardLog::addItem($member_id, $member_level, $second_reward, $share_config['second'], $second_total_num, 0, 2, 2);
                }
            }
            if ($share_config['third'] > 0) {//第三代投入金额的收益
                ...
                $third_reward = sprintf('%.6f', $third_total_num * ($share_config['third'] / 100));
                // 收益记录
                if ($third_reward > 0) {
                    ArkRewardLog::addItem($member_id, $member_level, $third_reward, $share_config['third'], $third_total_num, 0, 2, 3);
                }
            }
            $result = sprintf('%.6f', $first_reward + $second_reward + $third_reward);
        }
        Db::startTrans();
        try {
            $currency_id = ArkConfig::getValue('reward_currency_id');
            ...
            if ($result > 0) {
                ...
                $flag = ArkMember::where(['member_id' => $member_id])->setInc('share_reward', $result);
    ...
    static function team_reward($member_id, $money, $order_id) {
        ...
        try {
            ...
            foreach ($res as $key => $value) {
                ...
                if (!empty($income)) {
                    // 收益记录
                    $item_id = ArkRewardLog::addItem($currency_user['member_id'], $value['level'], $income, $rate, $money, $order_id, 3);
                    ...
                    $flag = ArkMember::where(['member_id' => $value['member_id']])->setInc('team_reward', $income);
    ...
    static function buy_detail($member_id, $type, $page, $rows=15) {
        ...
        $time_num = ArkConfig::getValue('cancel_time_num');
        foreach ($list as &$value) {
            ...
            $goods_info = ArkGoodsList::where(['goods_id' => $value['goods_id'], 'level' => keepPoint($value['level_num'] + $level_num - 1, 0), 'id' => ['gt', $value['goods_list_id']]])->find();
    ...
    static function get_order_index($member_id, $goods_id) {
        ...
        $list = self::alias('a')->where(['c.goods_id' => $goods_id, 'is_show' => 1])->where($where)
            ...
            ->join('ark_goods_list c', 'a.goods_list_id=c.id')
        ...
        foreach ($list as &$value) {
            ...
            $value['name'] = $ename . '推进' . round($value['money']) . 'Y令牌';
    ...
    static function get_order_list($member_id, $goods_id, $page, $rows=15) {
        $r = ['code' => ERROR1, 'message' => lang('no_data'), 'result' => null];
        $list_ids = ArkGoodsList::where(['goods_id' => $goods_id, 'is_show' => 1])->order('id desc')->limit(2)->column('id');
        $where = [];
        $special_user_id = ArkConfig::getValue('special_user_id');
        if ($special_user_id) {//判断是否特殊账号
            $special_user_id = explode(',', $special_user_id);
        ...
        foreach ($list as &$value) {
            $ename = mb_substr($value['ename'], 0, 2) . '**' . mb_substr($value['ename'], 4);
            $value['ename'] = $ename;
            $value['money'] = '推进' . round($value['money']) . 'Y令牌';
            $value['add_time'] = date('m-d H:i', $value['add_time']);
            if ($value['level_num'] == 1) {
                $value['title'] = '点火';
            } else {
                $value['title'] = '推进' . $value['level_num'];
            }
        }
    ...
    static function create_order($member_id, $product_id, $num, $kmt_num, $item) {
        ...
        if ($is_order) {
            $flag = ArkBuyList::where(['id' => $item['id']])->update(['status' => 99]);
...
//                }
        ...
        try {
            ...
            if ($num > 0) {
                ...
                if ($flag === false) throw new Exception('更新用户汇总失败');
                //更新队列信息
                $flag = ArkBuyList::where(['id' => $item['id']])->update(['status' => 1, 'actual_num' => $num, 'actual_kmt' => $kmt_num]);
                if ($flag === false) throw new Exception('更新队列信息失败');
            } else {
                //更新队列状态
                $flag = ArkBuyList::where(['id' => $item['id']])->update(['status' =>2]);
    ...
    static function sub_cancel($member_id, $order_id) {
        ...
        $goods_id = ArkGoodsList::where(['id' => $res['goods_list_id']])->value('goods_id');
        $level_num = self::getSettlementLevel($res['level_num']) - 1;
        $level_num = $res['level_num'] + $level_num;
        $goods_info = ArkGoodsList::where(['goods_id' => $goods_id, 'level' => $level_num, 'id' => ['gt', $res['goods_list_id']]])->find();
        $time_num = ArkConfig::getValue('cancel_time_num');
        if ($goods_info) {
            $check_time = keepPoint($goods_info['start_time'] - $time_num, 0);
            if ($check_time <= time() && $goods_info['start_time'] > time()) {
