...
class ArkBuyList extends Model
{
     ...
     * @param number $type     类型 1参与闯关 2预约闯关 3复利
     */
    static function add_list($member_id, $product_id, $num, $kmt_num, $type = 1) {
        ...
        $special_user_id = ArkConfig::getValue('special_user_id');//特殊账号
        if ($special_user_id) {
...
//                }
    ...
    static function handle_order($data) {
        foreach ($data as $key => $value) {
            $flag = ArkOrder::create_order($value['member_id'], $value['goods_list_id'], $value['num'], $value['kmt_num'], $value);
            if ($flag === false) {
