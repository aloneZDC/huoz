...
class RocketKmtLog extends Base
{
    /**
     * 新增记录
    ...
    static function addItem($member_id, $product_id, $num) {
        $info = RocketGoodsList::where(['id' => $product_id])->find();
    ...
    static function get_kmt_log($member_id, $page, $rows= 15) {
        ...
        foreach ($list as &$value) {
            ...
            if ($value['type'] == 7105) {//rocket_play_game
                $goods_list_id = RocketOrder::where(['id' => $value['third_id']])->value('goods_list_id');
                $info = RocketGoodsList::where(['id' => $goods_list_id])->find();
                if ($info) {
