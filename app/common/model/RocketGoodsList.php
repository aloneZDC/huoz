...
class RocketGoodsList extends Base
{
    /**
     * 添加子商品
    ...
    static function add_goods($goods_id, $type=1) {
        $data = RocketGoods::field('name,currency_id,price,min_payment,max_payment,profit,payment_rate,price_rate,start_hour,end_hour,start_time,run_daynum,lucky_Jackpot,fund_Jackpot,platform_Jackpot,kmt_rate,end_time')->where(['id' => $goods_id])->find();
...
//        $max_payment = keepPoint($data['max_payment'] + $payment,6);
    ...
    static function get_list($member_id, $goods_id, $page, $rows = 15) {
        ...
        $list = self::field('id,name,price,finish_money,min_payment,max_payment,profit,kmt_rate,last_settlement_time,start_hour,end_hour,rocket_status,level,status,start_time,end_time,run_daynum')
