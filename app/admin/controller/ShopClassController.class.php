...
class ShopClassController extends AdminController
{
    ...
    {
        $count = M('shop_category')->count();
        $Page = new \Think\Page($count, 12);
        $show = $Page->show();
        $list = M('shop_category')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        foreach ($list as $key => $vo) {
            $num = M('shop_goods')->field('count(*) as num')->where("cat_id=" . $vo['cat_id'])->select();
    ...
    public function class_add()
    {
        ...
        if (IS_POST) {
            ...
            if($_FILES['image']['size']  > 0){
                $data['image'] = $this->oss_upload($_FILES, 'images/shop/cates')['image'];
            }

            $rs = M('shop_category')->add($data);
            if ($rs) {
                $this->success("添加成功");
            } else {
                $this->error('添加失败');
    ...
    public function class_update()
    {
        ...
        if (!empty($id)) {
            $this->assign("id", $id);
            $where['cat_id'] = $id;
            $list = M('shop_category')->where($where)->find();
        }
        if (IS_POST) {
            ...
            //dump($data);
            $rs = M('shop_category')->where($wherec)->save($data);
            if ($rs) {
                $this->success("修改成功");
            } else {
                $this->error("修改失败");
