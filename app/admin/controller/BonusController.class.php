...
class BonusController extends AdminController {
    ...
    /**
     *  添加 分红奖励
     * @param unknown  Int  $currency_id   积分类型 id
    ...
    private  function getSumMoneyByCurrencyId($type,$currency_id){
    	...
    	$sql = "SELECT SUM(money)AS money,currency_id  from  yang_finance  where type = ".$type." 
    ...
    {
        $where['type'] = '1';
        $trade_fee_log = M('trade_fee_log');

        $count      = $trade_fee_log->where($where)->count();// 查询满足要求的总记录数
        $Page       = new \Think\Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数(25)
        //给分页传参数
        $show       = $Page->show();// 分页显示输出
        // 进行分页数据查询 注意limit方法的参数要使用Page类的属性
    ...
    {
        $where['type'] = '2';
        $red_packets_log = M('red_packets_log');

        $count      = $red_packets_log->where($where)->count();// 查询满足要求的总记录数
        $Page       = new \Think\Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数(25)
        //给分页传参数
        $show       = $Page->show();// 分页显示输出
        // 进行分页数据查询 注意limit方法的参数要使用Page类的属性

        $list = $red_packets_log
            ->where($where)
            ->limit($Page->firstRow.','.$Page->listRows)->order('add_time desc')->select();
