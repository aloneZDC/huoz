            ...
            if (!empty($v['trade_currency_id'])){
                ...
                foreach ($trade_currency_id as $vtId){
                    if(!empty( $coin_type[$vtId])){
                        ...
                        $trends = self::getTrends($v['currency_id'],$vtId,$from);//获取走势图
                        ...
                        // if($this->currency_id_mark[$vtId]=='XRP'){
...
//                                 }
    ...
    public function getTrends($currency_id,$toId='',$form=0)
    {
        ...
        if(empty($retult)){
            ...
            if (!empty($trade)) {
                for ($i = 0; $i < 35; $i++) {
                    if (isset($trade[$i]['max_price'])&&$trade[$i]['max_price'] != null) {
                        if(isset($trade[$i])){
...
//             $list[$k]['price'] = sprintf('%.4f',round($v['price'],4));
    ...
    {
        return db('Currency')->where('is_line=1 ')->order('sort ASC')->select();
    ...
    public  function getCurrencyMessageById($id,$toId='',$from=0)
    {
        ...
        if (empty($data)) {
            ...
            $list = !empty($this->currency_id_value[$id])?$this->currency_id_value[$id] :($Currency_model->where(array('currency_id' => $id,'is_line' => 1))->cache()->find());
            $time = time();
            //一天前的时间
...
//        $list = M('Member')->where(array('member_id' => session('USER_KEY_ID')))->find();
        $list = M('Member')->where(array('member_id' => session('USER_KEY_ID')))->find();
        if ($list['status'] == 0) {
            $msg = lang('lan_input_personal_info');
