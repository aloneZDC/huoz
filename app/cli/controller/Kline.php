...
class Kline extends Command
{
    ...
    {
        $this->setName('Kline')->setDescription('This is a Kline');
    }
...
//        }
    ...
    protected function every_minute() {
        $step = 60;
        $auto_trade_list = Db::name('currency_autotrade')->where(['is_autotrade'=>1,'kline_huobi'=>0,'kline_trade'=>0])->select();
    ...
    protected function history() {
        ...
        $auto_trade_list = Db::name('currency_autotrade')->where(['is_autotrade'=>1,'kline_huobi'=>0])->select();
        if(empty($auto_trade_list)) return;
    ...
    {
        $step = 60;
        $auto_trade_list = Db::name('currency_autotrade')->where(['is_autotrade' => 1, 'kline_huobi' => 1])->select();
        if (empty($auto_trade_list)) return;

        foreach ($auto_trade_list as $key => $value) {
            foreach ($this->time_list_huobi as $key1 => $value1) {
                $this->create_kline_huobi($value['currency_id'], $value['trade_currency_id'], $key1, $value1);
            }
        }
    }
     ...
     * @return string
     */
    protected function create_kline_huobi($currency_id, $currency_trade_id, $period, $type)
    {
        $where = [
        ...
        try {
            ...
            if (!$find) {
                ...
                if (!$this->create_kline_history_huobi($currency_id, $currency_trade_id, $symbol, $period, $type, $log)) {
     ...
     * @return string
     */
    protected function create_kline_history_huobi($currency_id, $currency_trade_id, $symbol, $period, $type,  &$log)
    {
    ...
    protected function every_minute_trade($start_day='') {
        ...
        $auto_trade_list = Db::name('currency_autotrade')->where(['is_autotrade'=>1,'kline_huobi'=>0,'kline_trade'=>1])->select();
        if(empty($auto_trade_list)) return;
    ...
    private function curl_get($url, $timeout = 30)
    {
