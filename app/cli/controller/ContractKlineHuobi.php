 ...
 * 用于实时接收火币K线数据
 */
class ContractKlineHuobi/* extends Server*/
{
    ...
    private $testip = array("172.18.242.235", "192.168.10.234");
    ...
    public function index()
    {
        ...
        foreach ($tradeList as $value) {
            ...
            foreach ($this->time_list as $k => $v) {
                ...
                echo "\r\n ".$info;
                $this->saveLog("huobi", $info);
                $r = \app\common\model\ContractKline::create_kline($value['id'], $value['currency_id'], $value['trade_currency_id'], $k, $v);
                if ($r['code'] == SUCCESS) {
        ...
        $this->worker->count = 1;
        $this->worker->name = 'ContractKlineHuobi';
                        ...
                        if (array_key_exists($period, $this->time_list)) {
                            ...
                            if ($find1) {//记录已存在，更新已有记录
                                if ($find1['open_price'] != $tick['open'] ||
                                    ...
                                    $kline = new \app\common\model\ContractKline;
