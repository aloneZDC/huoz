 ...
 * 用于实时接收火币K线数据
 */
class OrdersKline1/* extends Server*/
{
    ...
    public function index_old()
    {
        ...
        $this->server_host = KlineConfig::get_value('kline_local_server_url', '127.0.0.1:9999');
        ...
        $tradeList = Db::name('currency_autotrade')->where(['is_autotrade' => 1, 'kline_huobi' => 1, 'kline_id'=>$this->kline_id])->select();
        ...
        foreach ($tradeList as $value) {
            ...
            if (!array_key_exists($symbol, $this->trade_list)) {
                $this->trade_list[$symbol] = $value;
            }
...
//                echo "\r\n ".$info;
//                $this->saveLog("huobi", $info);
//                $r = \app\common\model\Kline::create_kline_huobi($value['currency_id'], $value['trade_currency_id'], $k, $v);
//                if ($r['code'] == SUCCESS) {
//
//                }
//            }
        ...
        $this->worker = new Worker();
        $this->worker->count = 1;
        $this->worker->name = 'OrdersKline'.$this->kline_id;
        $this->worker->onWorkerStart = function($worker)
        {
        ...
        $this->worker->onConnect = function($connection)
        {
            ...
            $connection->send(json_encode(array('Welcome to Turbo OrdersKline'.$this->kline_id.', id:'.$c_id)));
        };*/
    ...
    function onAsyncMessage($con, $message, $worker)
    {
        ...
        if(isset($data['ping'])) {
            ...
            switch ($pieces[2]) {
                case "kline":              //行情图
                    $market = $pieces[1];  //火币对
                    if (array_key_exists($market, $this->trade_list)) {
                        $period = $pieces[3];
                        if (array_key_exists($period, $this->time_list)) {
                            $tick = $data['tick'];
                            //tick 说明
                            //"tick": {
                            //  "id": K线id,
                            ...
                            //}
                            $type = $this->time_list[$period];
                            $id = $tick['id'];
                            $addRes = Db::name('kline_history')->insertGetId([
                                'type'=>$type,
                                'currency_id'=>$this->trade_list[$market]['currency_id'],
                                'currency_trade_id'=>$this->trade_list[$market]['trade_currency_id'],
                                'ch'=>$data['ch'],
                                'time_id'=>$id,
                                'amount'=>number_format($tick['amount'],6,".",""),
                                'count'=>number_format($tick['count'],6,".",""),
                                'open'=>number_format($tick['open'],6,".",""),
                                'close'=>number_format($tick['close'],6,".",""),
                                'low'=>number_format($tick['low'],6,".",""),
                                'high'=>number_format($tick['high'],6,".",""),
                            ...
                            if ($find1) {//记录已存在，更新已有记录
                                if ($find1['open_price'] != $tick['open'] ||
                                    ...
                                    $update_list[] = [
                                        ...
                                        'hign_price'=>number_format($tick['high'],6,".",""),
                                        'low_price'=>number_format($tick['low'],6,".",""),
                                        'num'=>number_format($tick['amount'],6,".",""),
                                        'amount'=>number_format($tick['amount'],6,".",""),
                                        'count'=>number_format($tick['count'],6,".",""),
                                        'vol'=>number_format($tick['vol'],6,".",""),
                                        'update_time'=>time(),
