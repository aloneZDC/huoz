...
use app\common\model\RocketDaySummary;
use app\common\model\RocketRewardLog;
use think\Log;
use think\Db;
...
class RocketTask extends Command
{
    protected $name = '闯关定时任务';
    ...
    {
        $this->setName('RocketTask')->setDescription('This is a RocketTask');
    }
    ...
    public function upgrade_level() {
        ...
        $member_info = RocketMember::order('member_id asc')->select();
        if ($member_info) {
            foreach ($member_info as $key => $value) {
                $flag = RocketMember::updateLevel($value['member_id']);
                if ($flag === false) {
                    Log::write($this->name . " 用户升级失败：" . $value['member_id']);
                }

                //合格考核
                $flag = RocketDaySummary::handle_reward($value['member_id'], $this->today_config['yestday_start']);
                if ($flag === false) {
    ...
    public function settlement_order() {
        ...
        $res = RocketGoodsList::where(['status' => 1, 'end_time' => ['elt', $this->today_config['today_start']], 'is_show' => 1])->order('id asc')->select();
        if (!$res){
            Log::write($this->name . " 闯关结算：没有正在运行的闯关");
            return;
        }
        foreach ($res as $key => $value) {
            $flag = RocketOrder::settlement_order($value, $this->today_config, $this->mining_config);
            if ($flag === false) {
    ...
    public function settlement_reward() {
        ...
        $res = \app\common\model\RocketSubscribeTransfer::where([
        ...
        foreach ($res as $key => $value) {
            $flag = RocketRewardLog::settlement_reward($value, $this->today_config);
            if ($flag === false) {
                Log::write('结算奖励失败：'.$value['id']);
            }

            $flag = \app\common\model\RocketSubscribeTransfer::where(['id' => $value['id'], 'is_settlement' => 0])->update(['is_settlement' => 1]);
            if ($flag === false) {
        ...
        //区县合伙人奖励
        $flag = RocketRewardLog::area_reward($this->today_config);
        if ($flag === false) {
            Log::write('区县合伙人奖励失败');
        }
    ...
    //结算预购分红
    public function settlement_special() {
        $res = \app\common\model\RocketSubscribeTransfer::where([
            'add_time' => ['between', [$this->today_config['yestday_start'], $this->today_config['yestday_stop']]],
            'type' => 1
        ])->sum('num');
        ...
        $flag = RocketRewardLog::settlement_special($res);
        if ($flag === false) {
            Log::write("添加预购分红失败");
            return;
        }
