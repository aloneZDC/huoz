...
class GacRelease extends Command
{
    protected function configure(){
        $this->setName('GacRelease')->setDescription('This is a AwardRelease');
    }
    ...
    protected function execute(Input $input, Output $output){
        ...
        if(empty($this->config) || empty($this->config['gac_forzen_release'])) {
            Log::write("GAC释放:获取配置失败");
            return;
        }

        if(empty($this->config['is_gac_release'])) {
            Log::write("GAC释放:后台关闭");
            return;
        }

        $currency = Db::name('currency')->field('currency_id,currency_mark,currency_name')->where(['currency_mark'=> 'GAC'])->find();
        if(!$currency) {
            Log::write("GAC释放:币种GAC查询失败");
            return;
        }
        $currency_id = $currency['currency_id'];
        if ($this->config['gac_forzen_release'] <= 0 || $this->config['gac_forzen_release'] > 1) {
        	Log::write("GAC释放:赠送释放后台设置错误");
        	return;
        }

        $today_begin = strtotime(date('Y-m-d'));
        $last_id = DB::name('currency_gac_forzen')->where('add_time>'.$today_begin)->max('third_id');
        while (true) {
        	$flag = false;

        	try{
            	Db::startTrans();
        		$cUser = Db::name('currency_user')->lock(true)->where('cu_id>'.$last_id.' and currency_id='.$currency_id.' and lock_num>0')->order('cu_id asc')->find();
        		if(!$cUser) {
        			$flag = true;
        			throw new Exception("GAC释放任务结束");
        		}

            	$money = keepPoint($cUser['lock_num'] * $this->config['gac_forzen_release'], 6);
            	if($money<0.000001) {
                    $last_id = $cUser['cu_id'];
                    throw new Exception("数值小于0.000001,取消释放GAC");
                }

            	//增加释放记录
	            $clf_id = Db::name('currency_gac_forzen')->insertGetId([
	                'member_id' => $cUser['member_id'],
	                'num' => $money,
	                'type' => 30,
                    'title' => 'lan_exchange_release_day',
                    'from_num' => $cUser['lock_num'],
	                'ratio' => ($this->config['gac_forzen_release']*100),
	                'add_time' => time(),
	                'third_id' => $cUser['cu_id'],
	            ]);
	            if(!$clf_id) throw new Exception("GAC释放记录添加失败");
                ...
                $result = model('AccountBook')->addLog([
                    ...
                    'type'=> 26,
                    'content' => 'lan_exchange_release_day',
                    'number_type' => 1,
                    'number' => $money,
                    'fee' => 0,
                    'third_id' => $clf_id,
