...
export default {
  ...
  data() {
    return {
      ...
      myActive: Number(localStorage.getItem("highIdx"))
        ? Number(localStorage.getItem("highIdx"))
        : 0,
      tabLabels: [], // 首页专区
  ...
  methods: {
    ...
    onLoad (type) {
      ...
      if(this[key].children_id.length <= 0) {
        // console.log(this[key].children_id);
        return;
      };
      this[tempUp] = true;
    ...
    tabHandler (idx) {
      ...
      // 存儲ID
      this.$nextTick(() => {
        localStorage.setItem("highIdx", idx);
      });
    ...
    async _index() {
      await this.$http.post(get_category, {type: "16"}).then(({ data }) => {
        if (data.code == 10000) {
          let dataArr = data.result;
          this.tabLabels = dataArr;
          this.info.title = "石墨烯区";
          this.tabLabels.forEach((item, index) => {
          switch (index) {
            case 0:
