                ...
                <div class="warpBox">
                  ...
                  >
                    ...
                    <div class="price_mi">
                      <div>赠与≈</div>
                      <div></div>
                      <div>{{ Math.round(item.goods_currency_give_num * item.hm_price) }}<span>&nbsp;CNY</span></div>
                    </div>
                  </div>
                </div>
              </van-list>
...
<script>
import { get_goods_list, get_category } from "@/http/api.js";
export default {
  name: "myShop",
  data() {
    return {
      info: {
        ...
        isBack: true,
        exit: true,
        shopDet: true, // 其他购物区类型删除localStorage
      },
      myActive: Number(localStorage.getItem("zeroIdx"))
        ? Number(localStorage.getItem("zeroIdx"))
        : 0,
      tabLabels: [], // 首页专区
  ...
  methods: {
    ...
    onLoad (type) {
      let key = type + "Option",
        ...
        isShow = "is" + type;
        if(this[key].children_id.length <= 0) {
          return;
        };
      this[tempUp] = true;
    ...
    tabHandler (idx) {
      ...
      // 存儲ID
      this.$nextTick(() => {
        localStorage.setItem("zeroIdx", idx);
      });
    ...
    async _index() {
      await this.$http.post(get_category, {type: "17"}).then(({ data }) => {
        if (data.code == 10000) {
          let dataArr = data.result;
          this.tabLabels = dataArr;
          this.info.title = "新人专区";
          this.tabLabels.forEach((item, index) => {
          switch (index) {
            case 0: 
...
.my-content {
  ...
  .content-bg {
    background: url("../../assets/shop/home/hy_home_bg.png") no-repeat center;
...
/deep/ header {
  background: url("../../assets/shop/home/hy_home_bgt.png") no-repeat center;
  background-size: 100% 100%;
  color: #fff;
  i {
    color: #fff;
