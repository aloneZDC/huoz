              ...
              <div class="listMain">
                <div class="main">
                  ...
                  <p style="display: none;">
                    燃料总量:<span>{{ item.price - 0 }}&nbsp;火米</span>
                  </p>
                  <p>
                  ...
                  <p class="topbox">
                    消耗MTK:<span>{{ item.kmt_rate - 0 }}&nbsp;%</span>
                  </p>
                </div>
              </div>
      ...
      <van-overlay :show="show">
        <div class="wrapper" v-if="payData">
          <div class="block">
            ...
            <p class="txt">燃料总量剩余:&nbsp;{{ payData.balance - 0 }} 火米</p>
            <div class="inputBox">
              <input
                type="text"
            ...
            <p class="consumption">
              消耗: <span>{{ payListId == 5 ? mtkNum : "0.0000" }}&nbsp;MTK</span>
            </p>
            ...
            <div class="payType" v-if="payListId">
              ...
              <div v-for="(item, index) in payList" :key="index" @click="changePay(item.id)" class="payTypeLists">
                ...
                <div>
                  {{ item.name }}{{item.id == 5 ? `(${item.num - 0} 火米+${payData.usre_kmt_num - 0} MTK)` : `可用(${item.num - 0} 火米)` }}
                </div>
              </div>
            </div>
            ...
            <!-- <div class="promptBox">
              <p>可用:{{ payData.user_num - 0 }}&nbsp;火米</p>
              <span @click="gofilling">去充值</span>
            </div>
            <p class="prompt">可用:{{ payData.usre_kmt_num - 0 }}&nbsp;MTK</p> -->
          </div>
        </div>
      </van-overlay>
    </div>
...
<script>
import { getList, get_pay_info, Pay, order_index, subscribe, user_info } from '@/http/api.js'
export default {
  data () {
    return {
      info: {
  ...
  methods: {
    ...
    getLists () {
      ...
      this.$http.post(order_index, opption).then(({ data }) => {
        if (data.code == 10000) {
    ...
    getpayFn () {
      ...
      this.$http.post(get_pay_info, this.payObj)
        .then(({ data }) => {
          if (data.code == '10000') {
    ...
    getPayInfo () {
      ...
      let num = this.cal.accMul(this.rlNum, this.kmtInfo.mtk_rate);
...
/deep/ .van-progress__pivot {
  background: url("../../assets/rocket/rocket.png") no-repeat center !important;
