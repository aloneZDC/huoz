    ...
    <div class="content">
      <div class="accountBox">
        <div class="account" v-if="infoData">
          ...
          <div class="accountNum">{{ infoData[1].num - 0 }} 火米</div>
        </div>
...
<script>  
import { transfer_info, add_transfer, transfer_log } from "@/http/api.js";
export default {
  name: "dets",
  components: {},
  data () {
  ...
  methods: {
    ...
    onLoad () {
      ...
      this.$http
        .post(transfer_log, this.dataOption)
        .then(({ data }) => {
    ...
    submitPay () {
      ...
      this.$http.post(add_transfer, obj)
        .then(({ data }) => {
        if (data.code == "10000") {
